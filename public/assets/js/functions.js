		...
		mercado_price_quantity: function(){
			if($(".quantity-input").length > 0){
				$(".quantity-input").on('click', '.btn', function(event) {
					event.preventDefault();
					var _this = $(this),
						_input = _this.siblings('input[name=product_qt]'),
						_current_value = _this.siblings('input[name=product_qt]').val(),
						_max_value = _this.siblings('input[name=product_qt]').attr('data-max');
